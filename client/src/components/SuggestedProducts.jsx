...
export default function SuggestedProducts() {
  ...
  useEffect(() => {
    async function loadProducts() {
      try {
        ...
        // Filter featured products (suggested) - available or out of stock
        const featuredProducts = productsWithBrands
  ...
  if (products.length === 0) {
    return null; // Don't show section if no suggested products
  }

  return (
    <section className="products-section">
      <div className="products-section-header">
        <h2 className="products-section-title">{t("shop.tabs.suggested")}</h2>
      </div>
